...
class VyOSParser(BaseConfigParser):
    ...
    def parse(self) -> CanonicalConfig:
        ...
        for number, raw in enumerate(self.lines, 1):
            ...
            if match := re.match(r"set interfaces ethernet (\S+)(?: vif (\d+))? (address|description) (.+)", line):
                ...
                else: iface.description = _unquote(value)
            ...
            if match := re.match(r"set firewall (?:ipv4 |ipv6 )?name (\S+) rule (\d+) (action|protocol|source address|destination address|source port|destination port|description) (.+)", line):
        ...
        for iface in interfaces.values():
            if not iface.segment_id:
                iface.segment_id = f"{device_id}-if-{slug(iface.name)}"; segments.append(Segment(id=iface.segment_id, name=iface.description or iface.name.upper(), type="vlan" if iface.vlan_id else "interface", device=device_id, vlan_id=iface.vlan_id, networks=interface_networks(iface.addresses)))
        ...
        for (name, sequence), data in firewall_rules.items():
            ...
            action = {"accept": "permit", "drop": "deny", "reject": "reject"}.get(data.get("action", ""), "unknown")
            ...
            policies.append(Policy(id=f"{device_id}:{name}:{sequence}", device=device_id, name=name, sequence=sequence,
                ...
                action=action, direction="zone" if source_zone else "unknown", from_zone=source_zone, to_zone=destination_zone,
